...
public class InactiveDeviceDetectionJob implements Job {
    ...
    private static final Integer UPDATE_BATCH_SIZE = Play.application().configuration().getInt("inactive_device_detection_job.update_batch_size");
    ...
    private void doJob(){
        ...
        try{
            ...
            if(deviceList != null){
                ...
                LOGGER.info("Inside InactiveDeviceDetectionJob - doJob(), Initiating the job for the device list with size - {}", deviceList.size());
                for (Device device : deviceList) {
                    try {
                        ...
                        iActCounts = deviceMetaDataMap.get(DeviceMetaAppendix.INACTIVE_PUSH_INTERVAL_COUNT) != null
                                ? Integer.parseInt(deviceMetaDataMap.get(DeviceMetaAppendix.INACTIVE_PUSH_INTERVAL_COUNT)) : DeviceMetaAppendix.INACTIVE_PUSH_INTERVAL_COUNT_DEFAULT_VALUE;

                        DeviceStatus temperatureStatus = deviceService.getOrCreateDeviceStatus(device, null, null, AssetStatusConstants.TEMP_STATUS_KEY, null);
                        final DeviceStatus activityStatus = deviceService.getOrCreateDeviceStatus(device, null, null, AssetStatusConstants.ACTIVITY_STATUS_KEY, null);

                        if (temperatureStatus.temperatureUpdatedTime == null || temperatureStatus.temperatureUpdatedTime == 0) {
                        ...
                        } else if (temperatureStatus.temperatureUpdatedTime < (currentTime - (iActCounts * pushInt))) {
                            activityStatus.status = AssetStatusConstants.ACTIVITY_STATUS_INACT;
                        ...
                        if (activityStatus.status.equals(AssetStatusConstants.ACTIVITY_STATUS_INACT)) {
                            ...
                            GenericAlarmRequest genericAlarmRequest = new GenericAlarmRequest(1, activityStatus.statusUpdatedTime);
                        ...
                        LOGGER.error("Error while running inactive device detection job for the device {}, {}", device.vendorId, device.deviceId, e);
                    }
                }
    ...
    private static void postDeviceAlarm(Map<String, AlarmLoggingRequest> alarmLoggingRequests) throws ServiceException {
