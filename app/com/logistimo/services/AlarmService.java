...
public class AlarmService extends ServiceImpl{
     ...
     * @param alarmLoggingRequest
     */
    public AlarmLoggingResponse postDeviceAlarm(AlarmLoggingRequest alarmLoggingRequest) throws ServiceException {
        ...
        for (AlarmRequest alarmRequest : alarmLoggingRequest.data) {
            try {
                ...
                for(DeviceAlarm deviceAlarm : deviceAlarmList){
                    ...
                    if(deviceAlarm.type != 3){
                        ...
                        if(deviceAlarm.sensorId != null){
                            deviceStatus = deviceService.getOrCreateDeviceStatus(device, null, deviceAlarm.sensorId, AssetStatusConstants.DEVICE_ALARM_STATUS_KEYS_MAP.get(deviceAlarm.type), null);
                        }else{
                            deviceStatus = deviceService.getOrCreateDeviceStatus(deviceAlarm.device, null, null, AssetStatusConstants.DEVICE_ALARM_STATUS_KEYS_MAP.get(deviceAlarm.type), null);
                        }
                        if(deviceStatus.status != deviceAlarm.status && (deviceStatus.statusUpdatedTime < deviceAlarm.time || deviceAlarm.type == AssetStatusConstants.ACTIVITY_ALARM_TYPE)){
                            ...
                            deviceEvent.type = DeviceEventPushModel.DEVICE_EVENT_ALARM_GROUP.get(deviceAlarm.type);
                        ...
                        try{
                            assetMapping = AssetMapping.findAssetMappingByRelatedAssetAndType(deviceAlarm.device, LogistimoConstant.MONITORED_BY);
                        ...
                        if(assetMapping != null){
                            ...
                            if(assetMapping.monitoringPositionId != null){
                                relatedDeviceStatus = deviceService.getOrCreateDeviceStatus(assetMapping.asset, assetMapping.monitoringPositionId, null, AssetStatusConstants.DEVICE_ALARM_STATUS_KEYS_MAP.get(deviceAlarm.type), null);
                            }else{
                                relatedDeviceStatus = deviceService.getOrCreateDeviceStatus(assetMapping.asset, null, null, AssetStatusConstants.DEVICE_ALARM_STATUS_KEYS_MAP.get(deviceAlarm.type), null);
                            }
                            ...
                            if(deviceAlarm.type == AssetStatusConstants.ACTIVITY_ALARM_TYPE) {
                                DeviceEventPushModel.DeviceEvent deviceEvent = new DeviceEventPushModel.DeviceEvent();
                            ...
                            deviceSet.add(assetMapping.asset);
                        }
                    }
                ...
                alarmLoggingResponse.errs.add(new ErrorResponse(alarmRequest.dId, alarmRequest.sId, e.getMessage()));
            } catch (NoResultException e) {
                LOGGER.warn("Error while posting alarm for device", e);
                alarmLoggingResponse.errs.add(new ErrorResponse(alarmRequest.dId, alarmRequest.sId, Messages.get(LogistimoConstant.DEVICES_NOT_FOUND)));
            }
        }
        ...
        if(!deviceEventPushModel.data.isEmpty()){
            try {
                ...
                LOGGER.error("{} while scheduling task for posting temperature event to Logistics service, {}", e.getMessage(), deviceEventPushModel.toString(), e);
            }
        }
     ...
     */
    public DeviceAlarmResponse getAlarm(String vendorId, String deviceId, String sid, int pageNumber, int pageSize) {
        ...
        List<DeviceAlarm> deviceAlarmList = DeviceAlarm.getDeviceAlarms(device, LogistimoUtils.transformPageNumberToPosition(pageNumber, pageSize), pageSize);
        for (DeviceAlarm deviceAlarm : deviceAlarmList) {
     ...
     * @return DeviceAlarm
     */
    private List<DeviceAlarm> toDeviceAlarms(AlarmRequest alarmRequest, Device device) throws LogistimoException {
        ...
        try{
            assetMappingList = AssetMapping.findAssetRelationByAssetAndType(device, LogistimoConstant.CONTAINS);
        }catch (NoResultException e){
            LOGGER.warn("Asset mapping not found for the device {}, {}", device.vendorId, device.deviceId, alarmRequest.dvc.iAct.toString());
        }
        ...
        if(alarmRequest.dvc.iAct != null){
            ...
            if(StringUtils.isNotEmpty(alarmRequest.sId)){
                ...
                if(assetMappingList != null && assetMappingList.size() == 1){
                    deviceAlarm.sensorId = LogistimoUtils.extractSensorId(assetMappingList.get(0).relatedAsset.deviceId);
                }
            }
            deviceAlarms.add(deviceAlarm);
            ...
            if(StringUtils.isNotEmpty(alarmRequest.sId)){
                try{
                    ...
                    deviceAlarm.time = alarmRequest.dvc.iAct.time;
                    deviceAlarm.type = DEVICE_NODATA_ALARM;
                    deviceAlarm.device = deviceService.findDevice(device.vendorId, LogistimoUtils.generateVirtualDeviceId(device.deviceId, alarmRequest.sId));
                    deviceAlarms.add(deviceAlarm);
                }catch (NoResultException e){
                ...
                if(assetMappingList != null && assetMappingList.size() == 1){
                    ...
                    LOGGER.warn("Too many relation/no relation found for the device {}, {}. Skipping alarm data propagation to virtual device {}", device.vendorId, device.deviceId, alarmRequest.dvc.iAct.toString());
                }
            }
        }
        ...
        if (alarmRequest.dvc.xSns != null) {
            ...
            if(StringUtils.isNotEmpty(alarmRequest.sId)){
                ...
                if(assetMappingList != null && assetMappingList.size() == 1){
                    ...
                    LOGGER.warn("Too many relation/no relation found for the device {}, {}. Skipping alarm data propagation to virtual device {}", device.vendorId, device.deviceId, alarmRequest.dvc.xSns.toString());
                }
            }
        }
     ...
     * @return DeviceAlarm
     */
    private List<AlarmLog> toAlarmLog(AlarmRequest alarmRequest, Device device) throws LogistimoException {
        List<AlarmLog> alarmLogs = new ArrayList<AlarmLog>();

        AlarmLog alarmLog;
        if (alarmRequest.dvc.batt != null) {
            closePreviousAlarmLog(AlarmLog.DEVICE_ALARM, BATTERY_ALARM, alarmRequest.dvc.batt.time, alarmRequest.sId,
                device);
        ...
        if (alarmRequest.dvc.dCon != null) {
            closePreviousAlarmLog(AlarmLog.DEVICE_ALARM, DEVICE_CONNECTION_ALARM, alarmRequest.dvc.dCon.time, alarmRequest.sId, device);
        ...
        if (alarmRequest.dvc.poa != null) {
            closePreviousAlarmLog(AlarmLog.DEVICE_ALARM, POWER_OUTAGE_ALARM, alarmRequest.dvc.poa.time, alarmRequest.sId, device);
        ...
        if (alarmRequest.dvc.xSns != null) {
            closePreviousAlarmLog(AlarmLog.DEVICE_ALARM, SENSOR_CONNECTION_ALARM, alarmRequest.dvc.xSns.time, alarmRequest.sId, device);
        ...
        if(alarmRequest.dvc.iAct != null){
            closePreviousAlarmLog(AlarmLog.DEVICE_ALARM, DEVICE_NODATA_ALARM, alarmRequest.dvc.iAct.time, alarmRequest.sId, device);
    ...
    private void closePreviousAlarmLog(Integer alarmType, Integer deviceAlarmType, Integer endTime, String sensorId, Device device) {
    ...
    public void generateAndPostAssetAlarm(Device device, String sId, Integer alarmType, Object alarmObject) throws ServiceException {
        ...
        switch (alarmType){
            ...
            case AssetStatusConstants.DEVICE_CONN_ALARM_TYPE:
                ...
                break;
            default:
                throw new ServiceException("Invalid alarmType " + alarmType + " while posting asset alarm for the device");
        }
