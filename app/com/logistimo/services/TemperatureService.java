...
import com.logistimo.models.alarm.request.*;
...
import com.logistimo.models.temperature.response.*;
...
public class TemperatureService extends ServiceImpl implements Executable{
    ...
    private static final String SUPPORT_STATUS_REQUEST_VENDORS = "logistimo.support.status_request.vendors";
    ...
    public void process(String content, Map<String, Object> options) throws Exception{
        if(content != null){
            ...
            TemperatureLoggingRequest temperatureLoggingRequest = LogistimoUtils.getValidatedObject(content, TemperatureLoggingRequest.class);
            TemperatureLoggingResponse temperatureLoggingResponse = logReadings(temperatureLoggingRequest, chSource);
            if (temperatureLoggingResponse.errs.size() == temperatureLoggingRequest.data.size()) {
                LOGGER.warn("No device(s) found: {}, and input data: {}" + temperatureLoggingResponse.toString(), temperatureLoggingRequest.toString());
            } else if (temperatureLoggingResponse.errs.size() > 0 || temperatureLoggingResponse.errTmps.size() > 0) {
                LOGGER.warn("Partial content: {}, data: {}", temperatureLoggingResponse.toString(), temperatureLoggingRequest.toString());
            } else {
    ...
    public TemperatureLoggingResponse logReadings(TemperatureLoggingRequest temperatureLoggingRequest, Integer chSource) throws ServiceException {
        ...
        for (DeviceReadingRequest deviceReadingRequest : temperatureLoggingRequest.data) {
            try {
                Device device = deviceService.findDevice(temperatureLoggingRequest.vId, deviceReadingRequest.dId);

                DevicePowerTransition devicePowerTransition = null;
                if(deviceReadingRequest.pwa != null){
                    devicePowerTransition = DevicePowerTransitionDAO.logPowerTransition(device, deviceReadingRequest.pwa);
                }

                List<String> errorReadings = logTemperatures(deviceReadingRequest.tmps, device, devicePowerTransition,chSource);
            ...
            if(mpId == null){
                ...
                for(AssetMapping assetMapping : assetMappingList){
                    temperatureReadingResponse.data = toTemperatureResponseList(
                            TemperatureReading.getReadingsBetweenByMA(device, assetMapping.monitoringPositionId, LogistimoUtils.transformPageNumberToPosition(pageNumber, pageSize), pageSize, from, to)
                    );
                }
            }else{
                temperatureReadingResponse.data = toTemperatureResponseList(
                        TemperatureReading.getReadingsBetweenByMA(device, mpId, LogistimoUtils.transformPageNumberToPosition(pageNumber, pageSize), pageSize, from, to)
                );
            }
    ...
    public TaggedTemperatureResponse getPaginatedReadingsByTag(String tagName, int pageNumber, int pageSize) {
        ...
        return toTaggedTemperatureResponse(
                ...
                LogistimoUtils.availableNumberOfPages(pageSize, TemperatureReading.getReadingsCountForDevices(deviceList)));
    }

    public TaggedTemperatureResponse toTaggedTemperatureResponse(List<TemperatureReading> temperatureReadingList, long pageCount) {
    ...
    private List<TemperatureReading> getTemperatureReading(Device device, int pageNumber, int pageSize, int from, int to) {
    ...
    private List<String> logTemperatures(List<TemperatureRequest> temperatureList, final Device rootDevice, DevicePowerTransition devicePowerTransition, Integer chSource) {
            ...
            try {
                if(StringUtils.isNotEmpty(temperature.sId)){
                    if(deviceMap.containsKey(temperature.sId)){
                        ...
                        currentDevice = deviceService.findDevice(rootDevice.vendorId, LogistimoUtils.generateVirtualDeviceId(rootDevice.deviceId, temperature.sId));
                    ...
                    try{
                        ...
                        if(assetMappingList != null && assetMappingList.size() == 1){
                            ...
                            LOGGER.warn("Too many relation found for the device {}, {}. Skipping temperature data {}", rootDevice.vendorId, rootDevice.deviceId, temperature.toString());
                            continue;
                        }
                    }catch (NoResultException e){
                        LOGGER.warn(
                ...
                if(temperatureReading != null){
                    ...
                    if(!sensorStatusUpdated.contains(currentDevice.deviceId)){
                        try{
                            ...
                            if(Objects.equals(deviceStatus.status, AssetStatusConstants.XSNS_ALARM_ALARM) && temperatureReading.timeOfReading > deviceStatus.statusUpdatedTime){
                                try{
                                    ExternalSensorAlarmsRequest xSns = new ExternalSensorAlarmsRequest(AssetStatusConstants.XSNS_ALARM_NORMAL, temperatureReading.timeOfReading);
                                    alarmService.generateAndPostAssetAlarm(rootDevice, sensorId, AssetStatusConstants.XSNS_ALARM_TYPE, xSns);
                                }catch (ServiceException e){
                                    LOGGER.warn("{} while generating sensor connected alarm for the device {}, {}", e.getMessage(), rootDevice.vendorId, rootDevice.deviceId, e);
                                }
                ...
                LOGGER.warn("Error while logging temperature for sensor, sensor({}) not found for the device: {}, {}", temperature.sId, currentDevice.vendorId, currentDevice.deviceId);
                errorReadings.add(temperature.toString());
            }catch (LogistimoException e) {
                LOGGER.warn("{}, while logging temperature for device: {}, {}", e.getMessage(), currentDevice.vendorId, currentDevice.deviceId, e);
                errorReadings.add(temperature.toString());
            }
        }
    ...
    private TemperatureReading saveTemperature(TemperatureRequest temperature, Device device, Device rootDevice, DevicePowerTransition devicePowerTransition, Integer source) throws LogistimoException {
        ...
        try{
            temperatureReading = TemperatureReading.getReading(device, temperature.time, temperature.typ, temperature.tmp);
        ...
        if(temperatureReading == null){
            ...
            try{
                assetMapping = AssetMapping.findAssetMappingByRelatedAssetAndType(deviceService.findDevice(device.vendorId, device.deviceId), LogistimoConstant.MONITORED_BY);
            }catch (NoResultException ignored){
                //do nothing
            }
            ...
            try{
                finalDevicePowerTransition = DevicePowerTransition.getRecentDevicePowerTransition(rootDevice, temperature.time);

                if(devicePowerTransition != null && devicePowerTransition.time > finalDevicePowerTransition.time){
    ...
    private void updateDeviceTemperatureStatus(Map<Device, TemperatureReading> mrTemperatureReadings){
        for(Device device : mrTemperatureReadings.keySet()){
            ...
            if(temperatureReading.timeOfReading > deviceStatus.temperatureUpdatedTime){
                Map<String, DeviceMetaData> metaDataMap = deviceService.getDeviceMetaDataMap(device, DeviceMetaAppendix.TMP_GROUP);
                ...
                if((deviceStatus.status.equals(AssetStatusConstants.TEMP_STATUS_NORMAL) && temperatureAbnormalStatus > AssetStatusConstants.TEMP_ABNORMAL_STATUS_NORMAL)
                        || (!Objects.equals(temperatureAbnormalStatus,
                    AssetStatusConstants.TEMP_ABNORMAL_STATUS_NORMAL) && deviceStatus.status > AssetStatusConstants.TEMP_STATUS_NORMAL && !Objects.equals(
                    ...
                    try {
                        ...
                        LOGGER.error("{} while generating task for temperature event processing for the device {}, {}", e.getMessage(), device.vendorId, device.deviceId, e);
                    }
    ...
    private void updateDeviceActivityStatus(Map<Device, TemperatureReading> mrTemperatureReadings){
        ...
        for(Device device : mrTemperatureReadings.keySet()){
            ...
            if(temperatureReading.timeOfReading > deviceStatus.statusUpdatedTime &&
                ...
                Map<String, String> deviceMetaDataMap = deviceService.getDeviceMetaDataValueAsString(device, DeviceMetaAppendix.INT_GROUP);
                ...
                int iActCounts = deviceMetaDataMap.get(DeviceMetaAppendix.INACTIVE_PUSH_INTERVAL_COUNT) != null ?
                        Integer.parseInt(deviceMetaDataMap.get(DeviceMetaAppendix.INACTIVE_PUSH_INTERVAL_COUNT)) :
                ...
                if(temperatureReading.timeOfReading >= (currentTime - pushInt * iActCounts)){
                    ...
                    try {
                        ...
                        LOGGER.error("{} while generating activity alarm for the device {}, {}", e.getMessage(), device.vendorId, device.deviceId, e);
                    }
                }
            }
        }
    ...
    private void generateAndPostActivityAlarm(Device device, DeviceStatus deviceStatus) throws ServiceException {
        AlarmLoggingRequest alarmLoggingRequest = new AlarmLoggingRequest(device.vendorId);
        GenericAlarmRequest genericAlarmRequest = new GenericAlarmRequest(deviceStatus.status, deviceStatus.statusUpdatedTime);
    ...
    private void updateRelatedAssetsTemperatureStatus(Map<Device, TemperatureReading> mrTemperatureReadings){
  ...
  public AssetMapping updateParentDeviceWithSensorTemperatureStatus(Device virtualDevice) {
    ...
    replicateDeviceStatus(assetMapping.asset, virtualDeviceAssetStatus, mainDeviceAssetStatus, true);
    return assetMapping;
  }
  ...
  private void updateMonitoredAssetStatus(Device relatedAsset, AssetMapping assetMapping,
        ...
        try{
            AlarmLog
                ...
                monitoredAssetStatus.statusUpdatedTime, monitoredAssetStatus.sensorId, AlarmLog.TEMP_ALARM, null);
        ...
        monitoredAssetStatus.temperatureAbnormalStatus = monitoringAssetStatus.temperatureAbnormalStatus;
        monitoredAssetStatus.temperature = monitoringAssetStatus.temperature;
        ...
        alarmLog.temperatureType = monitoredAssetStatus.status == 0 ? TemperatureEventType.INCURSION.getValue() : TemperatureEventType.EXCURSION.getValue();
        ...
        Map<String, String>
            assetMetaMap = deviceService.getDeviceMetaDataValueAsString(relatedAsset, DeviceMetaAppendix.TMP_GROUP);
        ...
        try {
          ...
          LOGGER.error(
              "{} while scheduling task for posting temperature event to Logistics service, {}", e.getMessage(), deviceEvent.toString(), e);
        }
    }
    ...
    private TemperatureReadingResponse toTemperatureReadingResponse(List<TemperatureReading> temperatureReadingList, long pageCount) {
        if (temperatureReadingList != null) {
            TemperatureReadingResponse temperatureReadingResponse = new TemperatureReadingResponse();
            for (TemperatureReading temperatureReading : temperatureReadingList) {
    ...
    public void getCurrentTemperature(String vendorId, String deviceId) throws LogistimoException {
        ...
        if(!Arrays.asList(Play.application().configuration()
            .getString(SUPPORT_STATUS_REQUEST_VENDORS).split(LogistimoConstant.SUPPORT_VENDOR_SEP)).contains(vendorId)){
        ...
        String phoneNumber = deviceService.getDeviceMetaDataValueAsString(device, DeviceMetaAppendix.GSM_SIM_PHN_NUMBER);
        if (StringUtils.isNotEmpty(phoneNumber)) {
            ...
            try {
                String request_url = Play.application().configuration().getString(SMS_STATUS_REQUEST_URL + "." + vendorId);
                Map<String, String> requestParams = new HashMap<String, String>();
                String countryCode = deviceService.getDeviceMetaDataValueAsString(device, DeviceMetaAppendix.LOCALE_COUNTRYCODE);
                if(countryCode == null){
            ...
            LOGGER.warn("Error while pushing Admin settings. Invalid mobile number for device: {}, {}", vendorId, deviceId);
