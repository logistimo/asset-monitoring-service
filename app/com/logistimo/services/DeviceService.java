...
import com.logistimo.models.device.request.*;
import com.logistimo.models.device.response.*;
...
public class DeviceService extends ServiceImpl{
    private static final String DEVICE_NOT_FOUND = "Device %s from vendor %s not yet created in the temperature system.";
    ...
    private static final TemperatureService temperatureService = ServiceFactory.getService(TemperatureService.class);
    ...
    public DeviceResponse getDevice(String vendorId, String deviceId) {
        ...
        return toDeviceResponse(device, deviceStatusList, deviceMetaDatas, assetUsers, assetMapModelMap);
    }

    public RecentAlertResponse getDeviceRecentAlarms(String vendorId, String deviceId, String sid, int page, int size){
        Device device = findDevice(vendorId, deviceId, sid);
        Integer alarmType = device.assetType.assetType.equals(AssetType.MONITORED_ASSET) ? AlarmLog.TEMP_ALARM : AlarmLog.DEVICE_ALARM;
        ...
        long nPages = LogistimoUtils.availableNumberOfPages(size, AlarmLog.getAlarmCountForDevice(device, alarmType));
    ...
    public Device findDevice(String vendorId, String deviceId) {
        try{
            ...
            LOGGER.warn(Messages.get(LogistimoConstant.DEVICES_NOT_FOUND) + ": {}, {}", vendorId, deviceId, e);
    ...
    public Device findDevice(String vendorId, String deviceId, String sensorId){
        return findDevice(vendorId, (sensorId != null && !sensorId.isEmpty()) ? LogistimoUtils.generateVirtualDeviceId(deviceId, sensorId) : deviceId);
    }
            ...
            try {
                if (deviceRequest.vId == null || deviceRequest.dId == null
                    ...
                    if(deviceRequest.mts != null){
                        ...
                        try{
                            List<AssetUser> assetUserList = AssetUser.getAssetUser(device, AssetUser.ASSET_MAINTAINER);
                            for(AssetUser assetUser : assetUserList){
                    ...
                    if(deviceRequest.ws != null){
                        DeviceStatus deviceStatus = getOrCreateDeviceStatus(device, null, null, AssetStatusConstants.WORKING_STATUS_KEY, null);
                        if(!deviceStatus.status.equals(deviceRequest.ws.st)){
                            ...
                            deviceStatus.update();
                            try{
                                DeviceStatusLog oldStatusLog = DeviceStatusLog.getDeviceStatusForGivenStartTime(device.id, AssetStatusConstants.WORKING_STATUS_KEY, oldStatusUpdatedTime);
                    ...
                    if(deviceRequest.meta != null){
                        try{
                            ...
                            Map<String, Object> result = new ObjectMapper().readValue(deviceRequest.meta.toString(), LinkedHashMap.class);
                            if(result != null){
                                constructAndUpdateDeviceMetaDataFromMap(deviceMetaDataList, LogistimoUtils.constructDeviceMetaDataFromJSON(null, result), device);
                            }
                        } catch (NoResultException ignored){
                            //do nothing
                        } catch (Exception e) {
                            LOGGER.error("{} while extracting device meta data from json {}, updating device {}, {} without meta data", e.getMessage(), deviceRequest.meta, deviceRequest.vId, deviceRequest.dId, e);
                        }
                    }

                    //Initializing asset status
                    for(String assetStatusKey : device.assetType.assetType.equals(AssetType.MONITORED_ASSET)
                            ? AssetStatusConstants.MONITORED_DEVICE_STATUS_KEYS : AssetStatusConstants.DEVICE_STATUS_KEYS){
                        if(device.assetType.assetType.equals(AssetType.MONITORED_ASSET)
                                && (assetStatusKey.equals(AssetStatusConstants.ACTIVITY_STATUS_KEY) || assetStatusKey.equals(AssetStatusConstants.TEMP_STATUS_KEY))){
                            if(deviceRequest.mps != null){
                                for(Integer monitoringPoint : deviceRequest.mps){
                                    getOrCreateDeviceStatus(device, monitoringPoint, null, assetStatusKey, assetStatusKey.equals(AssetStatusConstants.ACTIVITY_STATUS_KEY) ? AssetStatusConstants.ACTIVITY_STATUS_INACT : AssetStatusConstants.STATUS_OK);
                                }
                                ...
                                for(DeviceStatus deviceStatus : deviceStatusList){
                                    if(deviceStatus.statusKey.equals(assetStatusKey) && !deviceRequest.mps.contains(deviceStatus.locationId)){
                                        deviceStatus.delete();
                                    }
                                }
                            }
                            ...
                            for(TemperatureSensorRequest temperatureSensorRequest : deviceRequest.sns){
                                getOrCreateDeviceStatus(device, null, temperatureSensorRequest.getsId(), assetStatusKey, null);
                            }
                        }else {
    ...
    private void postDeviceStatus(Device device, DeviceStatus deviceStatus) {
        ...
        try {
            ...
            LOGGER.error("{} while scheduling task for posting device status update to Logistics service, {}", e.getMessage(), deviceEvent.toString(), e);
        }
    }
    ...
    public DeviceDeleteResponse deleteDevice(DeviceDeleteRequest deviceDeleteRequest) {
        ...
        for (String deviceId : deviceDeleteRequest.getdIds()) {
            try {
                ...
                LOGGER.warn(Messages.get(LogistimoConstant.DEVICES_NOT_FOUND) + ": {}, {}", deviceDeleteRequest.getvId(), deviceId, e);
    ...
    public DeviceConfigurationResponse getDeviceConfiguration(String vendorId, String deviceId, boolean isDevice) {
        Device device = findDevice(vendorId, deviceId);

        if(isDevice){
            DeviceStatus deviceStatus = null;
            try{
                deviceStatus = getOrCreateDeviceStatus(device, null, null, AssetStatusConstants.CONFIG_STATUS_KEY, null);
                deviceStatus.statusUpdatedTime = (int) (System.currentTimeMillis() / 1000);
                deviceStatus.status = 1;
                deviceStatus.update();
            } catch (NoResultException e){
                LOGGER.info("Configuration status not available for device: {}, {}", device.vendorId, device.deviceId);
            }
    ...
    public void addDeviceConfiguration(DeviceConfigurationRequest deviceConfigurationRequest) throws LogistimoException {
        Device device = null;
        ...
        if (device != null) {
            ...
            if(deviceConfigurationRequest.configuration != null){
                ...
                if(deviceConfigurationRequest.configuration.comm != null){
                    for(String key : DeviceMetaAppendix.INT_GROUP){
                        if(key.equals(DeviceMetaAppendix.INT_SINT)){
                            ...
                            createOrUpdateDeviceMetaData(device, DeviceMetaAppendix.INT_SINT, String.valueOf(sInt));
                        }else if(key.equals(DeviceMetaAppendix.INT_PINT)){
                            pInt = deviceConfigurationRequest.configuration.comm.pushInt;
                            createOrUpdateDeviceMetaData(device, DeviceMetaAppendix.INT_PINT, String.valueOf(pInt));
                        }
                    }
                }
                ...
                if(deviceConfigurationRequest.configuration.locale != null){
                    for(String key : DeviceMetaAppendix.LOCALE_GROUP){
                        if(key.equals(DeviceMetaAppendix.LOCALE_COUNTRYCODE)){
                            createOrUpdateDeviceMetaData(device, key, deviceConfigurationRequest.configuration.locale.cn);
                        }else if(key.equals(DeviceMetaAppendix.LOCALE_TIMEZONE)){
                            createOrUpdateDeviceMetaData(device, key, String.valueOf(deviceConfigurationRequest.configuration.locale.tz));
                        }
                    }
                }
                ...
                try{
                    List<AssetMapping> assetMappingList = AssetMapping.findAssetRelationByAssetAndType(device, LogistimoConstant.CONTAINS);
                    for(AssetMapping assetMapping : assetMappingList){
                        Integer sensorSamplingInt = sInt, sensorPushInt = pInt, sHighAlarmDur = highAlarmDur, sLowAlarmDur = lowAlarmDur,
                                sHighWarnDur = highWarnDur, sLowWarnDur = lowWarnDur;
                        Double sensorMax = max, sensorMin = min, sHighWarnTemp = highWarnTemp, sLowWarnTemp = lowWarnTemp;
                        if(deviceConfigurationRequest.configuration.sensors != null){
                            for(SensorConfigurationRequest sensor : deviceConfigurationRequest.configuration.sensors){
                                if(StringUtils.isNotEmpty(sensor.getsId())
                                        && sensor.getsId().equals(LogistimoUtils.extractSensorId(assetMapping.relatedAsset.deviceId))){
                                    if(sensor.comm != null){
                        ...
                        createOrUpdateDeviceMetaData(assetMapping.relatedAsset, DeviceMetaAppendix.INT_SINT, String.valueOf(sensorSamplingInt));
                        createOrUpdateDeviceMetaData(assetMapping.relatedAsset, DeviceMetaAppendix.INT_PINT, String.valueOf(sensorPushInt));
                        createOrUpdateDeviceMetaData(assetMapping.relatedAsset, DeviceMetaAppendix.TMP_MAX, String.valueOf(sensorMax));
                        createOrUpdateDeviceMetaData(assetMapping.relatedAsset, DeviceMetaAppendix.TMP_MIN, String.valueOf(sensorMin));

                        //Propagating alarm configuration to device
                        createOrUpdateDeviceMetaData(assetMapping.relatedAsset, DeviceMetaAppendix.ALARM_HIGH_DUR, String.valueOf(sHighAlarmDur));
                        createOrUpdateDeviceMetaData(assetMapping.relatedAsset, DeviceMetaAppendix.ALARM_HIGH_TEMP, String.valueOf(sensorMax));
                        createOrUpdateDeviceMetaData(assetMapping.relatedAsset, DeviceMetaAppendix.ALARM_LOW_DUR, String.valueOf(sLowAlarmDur));
                        createOrUpdateDeviceMetaData(assetMapping.relatedAsset, DeviceMetaAppendix.ALARM_LOW_TEMP, String.valueOf(sensorMin));
                        createOrUpdateDeviceMetaData(assetMapping.relatedAsset, DeviceMetaAppendix.WARN_HIGH_DUR, String.valueOf(sHighWarnDur));
                        createOrUpdateDeviceMetaData(assetMapping.relatedAsset, DeviceMetaAppendix.WARN_HIGH_TEMP, String.valueOf(sHighWarnTemp));
                        createOrUpdateDeviceMetaData(assetMapping.relatedAsset, DeviceMetaAppendix.WARN_LOW_DUR, String.valueOf(sLowWarnDur));
                        createOrUpdateDeviceMetaData(assetMapping.relatedAsset, DeviceMetaAppendix.WARN_LOW_TEMP, String.valueOf(sLowWarnTemp));
                    }
                }catch (NoResultException ignored){
                    //do nothing
                }
    ...
    public TaggedDeviceResponse getDevicesByTag(String tagName, String q, String assetType, Integer workingStatus, Integer alarmType, Integer alarmDuration,Integer awr, int pageNumber, int pageSize) {
        TaggedDeviceResponse taggedDeviceResponse = new TaggedDeviceResponse();
        List<Device> deviceList = getDeviceForTag(tagName, q, assetType, workingStatus, alarmType, alarmDuration,awr, LogistimoUtils.transformPageNumberToPosition(pageNumber, pageSize), pageSize);
        ...
        for (Device device : deviceList) {
            try {
                ...
                LOGGER.warn("No device meta data found for the device: {}, {}", device.vendorId, device.deviceId);
            }
            ...
            if(StringUtils.isEmpty(q)) {
                try {
                    ...
                    LOGGER.warn("No device status found for the device: {}, {}", device.vendorId, device.deviceId);
                }
                ...
                try {
                    ...
                    LOGGER.warn("No asset relation found for the device: {}, {}", device.vendorId, device.deviceId);
                }
            }

            taggedDeviceResponse.data.add(toDeviceResponse(device, deviceStatusList, deviceMetaDatas, assetUsers, assetMapModelMap));
        }

        if(deviceList.size() > 0){
            taggedDeviceResponse.setnAbnormalTemp(getAbnormalDeviceCount(deviceList));
            taggedDeviceResponse.setnAbnormalDevices(alarmService.getAbnormalAlarmCountForDevices(deviceList));
    ...
    public TaggedDeviceResponse getAbnormalDevicesByTags(String tagName, int pageNumber, int pageSize) {
        TaggedDeviceResponse taggedDeviceResponse = new TaggedDeviceResponse();
        List<Device> deviceList = getDeviceForTag(tagName, null, null, null, null, null,null, LogistimoUtils.transformPageNumberToPosition(pageNumber, pageSize), pageSize);
        List<DeviceMetaData> deviceMetaDatas = null;
        List<AssetUser> assetUsers = null;
        List<DeviceStatus> deviceStatusList = null;
        ...
        for (Device device : deviceList) {
            ...
            DeviceResponse deviceResponse = toDeviceResponse(device, deviceStatusList, deviceMetaDatas, assetUsers, null);
        ...
        taggedDeviceResponse.setnAbnormalDevices(getTempMetricsByTag(tagName, AssetType.ILR, AssetStatusConstants.TEMP_STATUS_ALARM));
    ...
    public TaggedDeviceCountResponse getTaggedDeviceCountResponse(String tagName) {
        ...
        taggedDeviceCountResponse.setnAbnormalDevices(getTempMetricsByTag(tagName, AssetType.ILR, AssetStatusConstants.TEMP_STATUS_ALARM));
    ...
    public List<TaggedDeviceCountResponse> searchDeviceStatusByTagPattern(String tagPattern){
        ...
        List<TaggedDeviceCountResponse> taggedDeviceCountResponses = new ArrayList<TaggedDeviceCountResponse>(1);
        ...
        for(Tag tag : tags){
            ...
            taggedDeviceCountResponse.setnAbnormalDevices(getTempMetricsByTag(tag.tagName, AssetType.ILR, AssetStatusConstants.TEMP_STATUS_ALARM));
            taggedDeviceCountResponse.setnIActDevices(getInactiveAssetCountByTag(tag.tagName, AssetType.ILR));
    ...
    private DeviceResponse toDeviceResponse(Device device, List<DeviceStatus> deviceStatusList,
                                            List<DeviceMetaData> deviceMetaDatas, List<AssetUser> assetUsers, Map<Integer, AssetMapModel> assetMapModelMap) {
        ...
        try{
            deviceResponse.sns = toTemperatureSensorRequests(TemperatureSensor.getTemperatureSensor(device));
        }catch (NoResultException ignored){
            //do nothing
        }
        ...
        if(deviceMetaDatas != null){
            ...
            deviceResponse.meta = Json.parse(LogistimoUtils.constructDeviceMetaJsonFromMap(metaDataMap).toString());
        }
                ...
                if(deviceStatus.statusKey.equals(AssetStatusConstants.TEMP_STATUS_KEY)){
                    ...
                    if(metaDataMap != null){
                        if(metaDataMap.containsKey(DeviceMetaAppendix.TMP_MIN)){
                            assetTemperatureResponse.min = Double.parseDouble(metaDataMap.get(DeviceMetaAppendix.TMP_MIN));
                        }

                        if(metaDataMap.containsKey(DeviceMetaAppendix.TMP_MAX)){
                            assetTemperatureResponse.max = Double.parseDouble(metaDataMap.get(DeviceMetaAppendix.TMP_MAX));
                        }
                    }
    ...
    private void createDevice(DeviceRequest deviceRequest) {
        if (deviceRequest != null) {
            ...
            try{
                Map<String, Object> result = new ObjectMapper().readValue(deviceRequest.meta.toString(), LinkedHashMap.class);
                if(result != null){
                    constructAndUpdateDeviceMetaDataFromMap(null, LogistimoUtils.constructDeviceMetaDataFromJSON(null, result), device);
                }
                ...
                deviceMetaData.value = String.valueOf(DeviceMetaAppendix.INACTIVE_PUSH_INTERVAL_COUNT_DEFAULT_VALUE);
                ...
                LOGGER.error("{} while extracting device meta data from json {}, creating device {}, {} without meta data", e.getMessage(), deviceRequest.meta, deviceRequest.vId, deviceRequest.dId, e);
            }
            ...
            for(String assetStatusKey : device.assetType.assetType.equals(AssetType.MONITORED_ASSET)
                ...
                if(device.assetType.assetType.equals(AssetType.MONITORED_ASSET)
                        && AssetStatusConstants.ASSET_SENSOR_STATUS_KEYS.contains(assetStatusKey)){
                    for(Integer monitoringPoint : deviceRequest.mps){
                        DeviceStatus deviceStatus = getOrCreateDeviceStatus(device, monitoringPoint, null, assetStatusKey, assetStatusKey.equals(AssetStatusConstants.ACTIVITY_STATUS_KEY) ? AssetStatusConstants.ACTIVITY_STATUS_INACT : AssetStatusConstants.STATUS_OK);
                    ...
                    DeviceStatus deviceStatus = getOrCreateDeviceStatus(device, null, null, assetStatusKey, null);
    ...
    private DeviceConfigurationResponse toDeviceConfigurationResponse(DeviceConfiguration deviceConfiguration) {
        DeviceConfigurationResponse deviceConfigurationResponse = new DeviceConfigurationResponse();
        deviceConfigurationResponse.data = new Gson().fromJson(deviceConfiguration.configuration, ConfigurationRequest.class);

        if(deviceConfigurationResponse.data != null && deviceConfigurationResponse.data.locale != null
                && deviceConfigurationResponse.data.locale.tznm != null && !deviceConfigurationResponse.data.locale.tznm.isEmpty()){
            TimeZone timeZone = TimeZone.getTimeZone(deviceConfigurationResponse.data.locale.tznm);
            deviceConfigurationResponse.data.locale.tz = (double) (timeZone.getRawOffset() + timeZone.getDSTSavings())/(60 * 60 * 1000);
        }
        return deviceConfigurationResponse;
    }

    private DeviceConfiguration toDeviceConfiguration(DeviceConfigurationRequest deviceConfigurationRequest, Device device, Tag tag) {
        DeviceConfiguration deviceConfiguration = new DeviceConfiguration();

        deviceConfiguration.configuration = Json.toJson(deviceConfigurationRequest.configuration).toString();
    ...
    private void updateDeviceConfiguration(DeviceConfiguration deviceConfiguration, DeviceConfigurationRequest deviceConfigurationRequest) {
        deviceConfiguration.configuration = Json.toJson(deviceConfigurationRequest.configuration).toString();
    ...
    private void updateDeviceConfigurationByTags(Set<Tag> tags, DeviceConfigurationRequest deviceConfigurationRequest, Device device) {
        DeviceConfiguration deviceConfiguration;
        for (Tag tag : tags) {
            try {
                if (device != null) {
    ...
    private TaggedDeviceResponse getPaginatedTaggedDeviceResponse(TaggedDeviceResponse taggedDeviceResponse, int pageNumber, int pageSize) {
        ...
        if (offset > taggedDeviceResponse.data.size()) {
            ...
            taggedDeviceResponse.data = taggedDeviceResponse.data.subList(offset, taggedDeviceResponse.data.size());
        } else {
    ...
    public void validateDeviceConfigurationRequest(DeviceConfigurationRequest deviceConfigurationRequest) throws LogistimoException {
        ...
        if (deviceConfigurationRequest.configuration.comm.chnl < 0 ||  deviceConfigurationRequest.configuration.comm.chnl > 2) {
            throw new LogistimoException("Communication channel(comm.chnl) is null/invalid.");
        }

        if (!LogistimoValidationUtils.validateURL(deviceConfigurationRequest.configuration.comm.tmpUrl)) {
            throw new LogistimoException("Temperature communication URL(comm.tmpUrl) is null/invalid.");
        }

        if (!LogistimoValidationUtils.validatePhoneNumber(deviceConfigurationRequest.configuration.comm.smsGyPh)) {
            throw new LogistimoException(Messages.get(LogistimoConstant.VALIDATION_CONFIG_SMSGYPH_NOTFOUND));
        }

        if (!LogistimoValidationUtils.validateDuration(deviceConfigurationRequest.configuration.comm.pushInt)) {
            throw new LogistimoException("Communication push interval(comm.pushInt) is null/invalid..");
        }

        if (!LogistimoValidationUtils.validateDuration(deviceConfigurationRequest.configuration.comm.samplingInt)) {
            throw new LogistimoException("Communication sampling interval(comm.samplingInt) invalid.");
        }

        if (deviceConfigurationRequest.configuration.highAlarm == null
                || !LogistimoValidationUtils.validateDuration(deviceConfigurationRequest.configuration.highAlarm.dur)
        ...
        if (deviceConfigurationRequest.configuration.comm.devAlrmsNotify
                && !LogistimoValidationUtils.validateURL(deviceConfigurationRequest.configuration.comm.alrmUrl)) {
        ...
        if (deviceConfigurationRequest.configuration.comm.statsNotify
                && !LogistimoValidationUtils.validateURL(deviceConfigurationRequest.configuration.comm.statsUrl)) {
     ...
     *
     * @param deviceReadyUpdateRequest
     * @return
     */
    public DeviceReadyUpdateResponse createOrUpdateDeviceReady(DeviceReadyUpdateRequest deviceReadyUpdateRequest) {
    ...
    private Device updateDeviceStatus(Device device, DeviceReadyRequest deviceReadyRequest){
        if (deviceReadyRequest.dev != null) {
            if(StringUtils.isNotEmpty(deviceReadyRequest.dev.mdl))
                createOrUpdateDeviceMetaData(device, DeviceMetaAppendix.DEV_MODEL_FROM_DEVICE, deviceReadyRequest.dev.mdl);

            if(StringUtils.isNotEmpty(deviceReadyRequest.dev.getdVr()))
                createOrUpdateDeviceMetaData(device, DeviceMetaAppendix.DEV_DVR, deviceReadyRequest.dev.getdVr());

            if(StringUtils.isNotEmpty(deviceReadyRequest.dev.getmVr()))
                createOrUpdateDeviceMetaData(device, DeviceMetaAppendix.DEV_MVR, deviceReadyRequest.dev.getmVr());

            if(StringUtils.isNotEmpty(deviceReadyRequest.dev.imei))
                createOrUpdateDeviceMetaData(device, DeviceMetaAppendix.DEV_IMEI, deviceReadyRequest.dev.imei);
        }

        if (deviceReadyRequest.sim != null) {
            if(StringUtils.isNotEmpty(deviceReadyRequest.sim.sid))
                createOrUpdateDeviceMetaData(device, DeviceMetaAppendix.GSM_SIM_SIMID, deviceReadyRequest.sim.sid);

            if(StringUtils.isNotEmpty(deviceReadyRequest.sim.phn))
                createOrUpdateDeviceMetaData(device, DeviceMetaAppendix.GSM_SIM_PHN_NUMBER, deviceReadyRequest.sim.phn);
        }

        if(deviceReadyRequest.altSim != null){
            if(StringUtils.isNotEmpty(deviceReadyRequest.altSim.sid)){
                createOrUpdateDeviceMetaData(device, DeviceMetaAppendix.GSM_ALTSIM_SIMID, deviceReadyRequest.altSim.sid);
            }

            if(StringUtils.isNotEmpty(deviceReadyRequest.altSim.phn)){
                createOrUpdateDeviceMetaData(device, DeviceMetaAppendix.GSM_ALTSIM_PHN_NUMBER, deviceReadyRequest.altSim.phn);
            }
        }
    ...
    public void pushConfigToDevice(ConfigurationPushRequest configurationPushRequest) throws LogistimoException, IOException {
        DeviceConfigurationPushStatus deviceConfigurationPushStatus = new DeviceConfigurationPushStatus();
        try {
            ...
            String countryCode = getDeviceMetaDataValueAsString(device, DeviceMetaAppendix.LOCALE_COUNTRYCODE);
            if (countryCode == null) {
                countryCode = LogistimoConstant.DEFAULT_COUNTRY_CODE;
            }

            if(!Arrays.asList(Play.application().configuration().getString(SUPPORT_CONFIG_PUSH_VENDORS).split(LogistimoConstant.SUPPORT_VENDOR_SEP)).contains(configurationPushRequest.vId)){
            ...
            String phoneNumber = getDeviceMetaDataValueAsString(device, DeviceMetaAppendix.GSM_SIM_PHN_NUMBER);
            if (phoneNumber != null) {
                try {
                    String requestUrl = Play.application().configuration().getString(CONFIG_PUSH_URL + "." + device.vendorId);
                    if (configurationPushRequest.typ == 1 && configurationPushRequest.data == null) {
                    ...
                    } else if (configurationPushRequest.typ == 0 && (configurationPushRequest.url == null || configurationPushRequest.url.isEmpty())) {
                        ...
                        if(deviceConfiguration != null
                            ...
                            LOGGER.info("Configuration/Configuration URL not defined for device: {} {}, picking system configuration pull URL.", device.deviceId, device.vendorId);
                            configurationPushRequest.url = Play.application().configuration().getString(CONFIG_PULL_URL);
                        }
                    }
            ...
            LOGGER.warn("Error while pushing configuration, device not found: {}, {}", configurationPushRequest.vId,
    ...
    public void pushAPNSettingsToDevice(APNPushRequest apnPushRequest) throws LogistimoException {
        ...
        if(!Arrays.asList(Play.application().configuration().getString(SUPPORT_APN_PUSH_VENDORS).split(LogistimoConstant.SUPPORT_VENDOR_SEP)).contains(apnPushRequest.getvId())){
        ...
        if (phoneNumber != null) {
            try {
                String request_url = Play.application().configuration().getString(APN_PUSH_URL + "." + apnPushRequest.getvId());
                ...
                try{
                    ...
                    LOGGER.warn("APN Settings not found, creating new for device: {}, {}", device.vendorId, device.deviceId);
                }
                ...
                LOGGER.error("Error while pushing APN settings for device: {}, {}", device.vendorId, device.deviceId, e);
    ...
    public AdminPushRequest getAdminSettings(String vendorId, String deviceId){
        try{
            ...
            try{
                ...
                LOGGER.warn("Error while getting Admin Settings, no admin settings for device: {}, {}", vendorId, deviceId, e);
                throw new NoResultException(Messages.get(DEVICE_NOT_FOUND));
            }
        }catch (NoResultException e){
            LOGGER.warn("Error while getting admin Settings, device: {} {} not found", vendorId, deviceId, e);
            throw new NoResultException(Messages.get(DEVICE_NOT_FOUND));
        }
    }

    public void pushAdminSettingsToDevice(AdminPushRequest adminPushRequest) throws LogistimoException {
        Device device = findDevice(adminPushRequest.getvId(), adminPushRequest.getdId());
        DeviceAdminSettingsPushStatus deviceAdminSettingsPushStatus = new DeviceAdminSettingsPushStatus();
        ...
        if(!Arrays.asList(Play.application().configuration().getString(SUPPORT_ADMIN_PUSH_VENDORS).split(LogistimoConstant.SUPPORT_VENDOR_SEP)).contains(adminPushRequest.getvId())){
        ...
        if (phoneNumber != null) {
            try {
                String request_url = Play.application().configuration().getString(ADMIN_PUSH_URL + "." + adminPushRequest.getvId());
                Map<String, String> requestParams = new HashMap<String, String>();
                requestParams.put(LogistimoConstant.COUNTRY_CODE_PARAM, countryCode);
                requestParams.put(LogistimoConstant.PHONE_PARAM, URLEncoder.encode(phoneNumber, "UTF-8"));
                request_url = HttpUtil.replace(request_url, requestParams);
                ...
                LOGGER.error("Error while pushing Admin settings. For device: {}, {}", adminPushRequest.getvId(),
    ...
    public void updateConfigSentStatus(DeviceSMSStatusRequest deviceSMSStatusRequest) throws LogistimoException {
        try{
            Device device = findDevice(deviceSMSStatusRequest.getvId(), deviceSMSStatusRequest.getdId());

            DeviceConfigurationPushStatus deviceConfigurationPushStatus = DeviceConfigurationPushStatus.findSentByDevice(device);
    ...
    public void updateAdminSentStatus(DeviceSMSStatusRequest deviceSMSStatusRequest) throws LogistimoException {
        try{
            Device device = findDevice(deviceSMSStatusRequest.getvId(), deviceSMSStatusRequest.getdId());

            DeviceAdminSettingsPushStatus deviceAdminSettingsPushStatus = DeviceAdminSettingsPushStatus.findSentByDevice(device);
    ...
    private void toDeviceAPNSettings(DeviceAPNSettings deviceAPNSettings, APNPushRequest apnPushRequest, Device device){
    ...
    private APNPushRequest toApnPushRequest(DeviceAPNSettings deviceAPNSettings){
        ...
        String countryCode = getDeviceMetaDataValueAsString(deviceAPNSettings.device, DeviceMetaAppendix.LOCALE_COUNTRYCODE);
        String phoneNumber = getDeviceMetaDataValueAsString(deviceAPNSettings.device, DeviceMetaAppendix.GSM_SIM_PHN_NUMBER);
    ...
    private void toDeviceAdminSettings(DeviceAdminSettings deviceAdminSettings, AdminPushRequest adminPushRequest, Device device){
    ...
    private List<AlertResponse> toRecentAlertResponses(List<AlarmLog> alarmLogs){
        ...
        for(AlarmLog alarmLog : alarmLogs){
            ...
            if(alarmLog.alarmType == AlarmLog.DEVICE_ALARM){
                alertResponse.devalm = new AlarmResponse(alarmLog.deviceAlarmType, alarmLog.deviceAlarmStatus != null ? alarmLog.deviceAlarmStatus : -1, alarmLog.deviceFirmwareErrorCode, alarmLog.sensorId);
            }
    ...
    private void createOrUpdateTemperatureSensors(Device device, DeviceRequest deviceRequest){
        for(TemperatureSensorRequest temperatureSensorRequest : deviceRequest.sns){
            ...
            try{
                temperatureSensor = TemperatureSensor.getTemperatureSensor(device, temperatureSensorRequest.getsId());
            ...
            try{
                sensorDevice = Device.findDevice(device.vendorId, LogistimoUtils.generateVirtualDeviceId(device.deviceId, temperatureSensorRequest.getsId()));

                //Propagating parent device model to sensors devices.
                createOrUpdateDeviceMetaData(sensorDevice, DeviceMetaAppendix.DEV_MODEL, getDeviceMetaDataValueAsString(device, DeviceMetaAppendix.DEV_MODEL));
            }catch (NoResultException ignored){
                //do nothing
            }
            ...
            if(sensorDevice == null){
                ...
                sensorDevice.deviceId = LogistimoUtils.generateVirtualDeviceId(device.deviceId, temperatureSensorRequest.getsId());
    ...
    private List<TemperatureSensorRequest> toTemperatureSensorRequests(List<TemperatureSensor> temperatureSensors){
    ...
    private void updateTemperatureSensorStatus(Device device, List<String> actSns){
        try{
            ...
            LOGGER.warn("Sensor not found for the device: {}, {}. But received active sensor: {}", actSns.toArray());
        }
    }
    ...
    private void constructAndUpdateDeviceMetaDataFromMap(List<DeviceMetaData> deviceMetaDataList, Map<String, String> deviceMetaDataMap, Device device){
    ...
    public void createOrUpdateAssetMapping(AssetRegistrationRelationModel assetRegistrationRelationModel){
        if(assetRegistrationRelationModel != null && assetRegistrationRelationModel.data != null){
            for(AssetRelationModel assetRelationModel : assetRegistrationRelationModel.data){
                try{
                            ...
                            try{
                                ...
                                for(AssetMapping assetMapping : assetMappingList){
                                    if(monitoringDevice == null) {
                                        String deviceId = LogistimoUtils.extractDeviceId(assetMapping.relatedAsset.deviceId);
                            ...
                            for(AssetMapModel assetMapModel : assetRelationModel.ras){
                                try{
                                    ...
                                    String mappingAssetId = isSensor
                                            ? LogistimoUtils.generateVirtualDeviceId(assetMapModel.getdId(), assetMapModel.getsId())
                                            : assetMapModel.getdId();
                                    Device monitoringDevice = findDevice(assetMapModel.getvId(), mappingAssetId);
                                    try{
                                        AssetMapping assetMapping = AssetMapping.findAssetMappingByAssetAndMonitoringPosition(device, assetMapModel.mpId);
                                    ...
                                    for(String statusKey : AssetStatusConstants.ASSET_SENSOR_STATUS_KEYS) {
                                        try{
                                            monitoringAssetStatus = DeviceStatus.getDeviceStatus(monitoringDevice, statusKey);
                                            monitoredAssetStatus = DeviceStatus.getDeviceStatus(device, statusKey, assetMapModel.mpId);
                                            if(statusKey.equals(AssetStatusConstants.TEMP_STATUS_KEY)) {
                                                monitoringAssetStatus.status = AssetStatusConstants.STATUS_OK;
                                                monitoringAssetStatus.temperatureAbnormalStatus = AssetStatusConstants.STATUS_OK;
                                            ...
                                            LOGGER.warn("Device status not found: {}, {}", assetMapModel.getvId(), assetMapModel.getdId());
                                        }
                                    }
                                    ...
                                    parentDeviceStatus = DeviceStatus.getDeviceStatus(parentDevice, AssetStatusConstants.TEMP_STATUS_KEY, assetMapModel.getsId());
                                    ...
                                    LOGGER.warn("Mapping device not found: {}, {}", assetMapModel.getvId(), assetMapModel.getdId());
                                }
                            }
                    ...
                    LOGGER.warn("Device not found while mapping device {}, {}", assetRelationModel.vId, assetRelationModel.dId);
                }
            }
        }
    }
    ...
    private void createAssetMapping(AssetMapModel assetMapModel, Device device, Device mappingDevice){
        ...
        if(Objects.equals(assetMapModel.typ, LogistimoConstant.MONITORED_BY)){
            for(String assetStatusKey : AssetStatusConstants.MONITORING_ASSET_STATUS_KEYS){
                getOrCreateDeviceStatus(device, assetMapping.monitoringPositionId, null, assetStatusKey, null);
            }
        }
    }
    ...
    private Map<Integer, AssetMapModel> toAssetMapModels(List<AssetMapping> assetMappingList){
        ...
        for(AssetMapping assetMapping : assetMappingList){
            ...
            try{
                ...
                LOGGER.warn("No device meta data found for the device: {}, {}", assetMapping.relatedAsset.vendorId, assetMapping.relatedAsset.deviceId);
            }

            assetMapModel.setdId(assetMapping.relatedAsset.assetType.id.equals(AssetType.TEMP_SENSOR) ? LogistimoUtils.extractDeviceId(assetMapping.relatedAsset.deviceId) : assetMapping.relatedAsset.deviceId);
            ...
            assetMapModel.setsId(assetMapping.relatedAsset.assetType.id.equals(AssetType.TEMP_SENSOR) ? LogistimoUtils.extractSensorId(assetMapping.relatedAsset.deviceId) : null);
            ...
            if(deviceMetaDatas != null){
                ...
                assetMapModel.meta = Json.parse(LogistimoUtils.constructDeviceMetaJsonFromMap(metaDataMap).toString());
            }
            assetRelationMap.put(assetMapping.monitoringPositionId != null ? assetMapping.monitoringPositionId : index, assetMapModel);
            index ++;
        }
        return assetRelationMap;
    }

    public DeviceStatus getOrCreateDeviceStatus(Device device, Integer monitoringPositionId, String sensorId, String key, Integer status){
        DeviceStatus deviceStatus = null;
    ...
    public Integer getInactiveAssetCountByTag(String tagName, Integer assetType){
        ...
        return new BigInteger(JPA.em().createNativeQuery(innerQueryStr).getSingleResult().toString()).intValue();
    }
    ...
    public Integer getTempMetricsByTag(String tagName, Integer assetType, Integer status){
        ...
        String innerQueryStr = "select count(1) from(" +
                "  select distinct device_id from device_status where status_key in :inclList and status =  " + status + " and device_id in(" +
        ...
        return new BigInteger(JPA.em().createNativeQuery(innerQueryStr).setParameter("inclList", statusKeyList).getSingleResult().toString()).intValue();
    }

    public Integer getAssetsCountForTag(String tagName, Integer assetType){
        String innerQueryStr = "select count(1) from devices where assetType_id = " + assetType + " and id in("
                + "select devices_id from devices_tags where tags_id in("
                + "select id from tags where tagname = '" + tagName + "'))";
    ...
    public List<Device> getDeviceForTag(String tagName, String q, String assetType, Integer workingStatus, Integer alarmType, Integer alarmDuration, Integer awr,
                                        Integer startingOffset, Integer size){
        String innerQueryStr = "select devices_id from devices_tags where tags_id in(select id from tags where tagname = '" + tagName + "')";

        if(workingStatus != null && workingStatus != -1){
            innerQueryStr = "select device_id from device_status where status_key = '" + AssetStatusConstants.WORKING_STATUS_KEY
        ...
        if(alarmType != null){
            ...
            if(alarmType == 1){
                ...
                innerQueryStr = "select device_id from device_status where status_key = '" + AssetStatusConstants.TEMP_STATUS_KEY
                        + "' and status = 3"
                ...
                /*innerQueryStr = "select DISTINCT device_id from device_status where status_key = 'dsk_4' and device_id in(" + innerQueryStr + ")"
                        + "group by device_id having max(status) = 0";*/
                innerQueryStr = "select DISTINCT device_id from device_status where status_key in (" + AssetStatusConstants.DEVICE_ALARM_STATUS_KEYS_CSV
                        + ") and status > 0"
                        + " and status_ut <= " + alarmSince
                        + " and device_id in(" + innerQueryStr + ")";
            }else if(alarmType == 3){
                innerQueryStr = "select DISTINCT device_id from device_status where status_key = '" + AssetStatusConstants.ACTIVITY_STATUS_KEY
                        + "' and status > 0"
                        + " and status_ut <= " + alarmSince
                        + " and device_id in(" + innerQueryStr + ")";
            }else if(alarmType == 4){
                ...
                if(Objects.equals(AssetType.getAssetType(aty).assetType, AssetType.MONITORING_ASSET)){
                    innerQueryStr = "select DISTINCT device_id from device_status where status_key in (" + AssetStatusConstants.DEVICE_ALARM_STATUS_KEYS_CSV
                            + ")"
                            + " and device_id in(" + innerQueryStr + ") group by device_id having max(status) = 0";
                }else{
                    innerQueryStr = "select DISTINCT device_id from device_status where status_key = '" + AssetStatusConstants.TEMP_STATUS_KEY + "'"
                            + " and device_id in(" + innerQueryStr + ") group by device_id having max(status) < 3";
                }
            }
        }
        ...
        if(awr > 0){
            if(AssetType.MONITORED_ASSET.equals(AssetType.getAssetType(Integer.parseInt(assetType.split(",")[0])).assetType)) {
                queryStr += "AND " + (awr == 2 ? "NOT" : "") + " EXISTS(select 1 from asset_mapping where asset_id=d.id and relation_type=2 limit 1)";
            } else {
                queryStr += "AND " + (awr == 2 ? "NOT" : "") + " EXISTS(select 1 from asset_mapping where relation_type = 2 and relatedAsset_id in (select relatedAsset_id from asset_mapping where asset_id=d.id and relation_type=1) limit 1)";
            }
        }
    ...
    public AssetPowerTransitions getDevicePowerTransition(String vendorId, String deviceId, Integer from, Integer to){
        return buildAssetPowerTransitions(DevicePowerTransition.getDevicePowerTransition(findDevice(vendorId, deviceId), from, to));
    }

    public AssetPowerTransitions buildAssetPowerTransitions(List<DevicePowerTransition> devicePowerTransitions){
        ...
        for(DevicePowerTransition devicePowerTransition : devicePowerTransitions){
            AssetPowerTransitions.AssetPowerTransition assetPowerTransition = new AssetPowerTransitions.AssetPowerTransition();
        ...
        try {
            ...
            for (DeviceStatus status : statuses) {
                if (status.sensorId == null && status.locationId == null) {
                    ...
                    finalStatus = status;
                } else if (Objects.equals(statusKey, AssetStatusConstants.ACTIVITY_STATUS_KEY) && status.status < finalStatus.status) {
                    finalStatus = status;
                }
            }
            ...
            if (finalStatus != null) {
                ...
                if (!Objects.equals(currentOverallStatus.status, finalStatus.status) || !Objects.equals(currentOverallStatus.temperatureAbnormalStatus,
     ...
     * @param deviceStatus
     * @param oldStatusUpdatedTime
     * @param oldStatus
     * @param device
     */
    public void createAlarmLog(DeviceStatus deviceStatus, Integer oldStatusUpdatedTime, Integer oldStatus, Device device) {
        if(deviceStatus != null && oldStatusUpdatedTime != null && oldStatus != null && device != null) {
            AlarmLog newAlarmLog;
            Integer alarmType = deviceStatus.statusKey.equals(AssetStatusConstants.TEMP_STATUS_KEY) ? AlarmLog.TEMP_ALARM : AlarmLog.DEVICE_ALARM;
            Integer deviceAlarmType = deviceStatus.statusKey.equals(AssetStatusConstants.TEMP_STATUS_KEY) ? null : getDeviceAlarmType(deviceStatus);
            ...
            try{
                if(StringUtils.isNotEmpty(deviceStatus.sensorId)) {
                    ...
                    AlarmLog parentAlarmLog = AlarmLog.getParentAlarmLog(device, alarmType, deviceAlarmType, oldStatusUpdatedTime);
            ...
            if(deviceStatus.statusKey.equals(AssetStatusConstants.TEMP_STATUS_KEY)) {
                ...
                newAlarmLog.temperatureType = fetchAlarmTypeForDeviceStatus(deviceStatus.statusKey, deviceStatus.status);
                newAlarmLog.temperature = deviceStatus.temperature;
            } else {
                newAlarmLog = new AlarmLog(alarmType, deviceStatus.statusUpdatedTime);
                newAlarmLog.deviceAlarmStatus = fetchAlarmTypeForDeviceStatus(deviceStatus.statusKey, deviceStatus.status);
            }
