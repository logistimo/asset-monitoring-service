...
public class TemperatureEventService extends ServiceImpl implements Executable {
    ...
    public void process(String object, Map<String, Object> options) throws Exception{
        if(options != null && options.get(EVENT_TYPE) != null && options.get(STATE_UPDATED_TIME) != null
                && options.get(VENDOR_ID) != null && options.get(DEVICE_ID) != null){
            LockUtil.LockStatus lockStatus = LockUtil.lock(LogistimoConstant.DEVICE_LOCK + LogistimoUtils.extractDeviceId((String) options.get(DEVICE_ID)));
            try{
                if(!LockUtil.isLocked(lockStatus)){
                    throw new ServiceException("Error while locking the device " + LogistimoUtils.extractDeviceId((String) options.get(DEVICE_ID)));
                }

                Device device = deviceService.findDevice((String) options.get(VENDOR_ID), (String) options.get(DEVICE_ID));
                ...
                if(deviceStatus.statusUpdatedTime.equals(options.get(STATE_UPDATED_TIME))) {
                    if (options.get(EVENT_TYPE) == TemperatureEventType.EXCURSION) {
                        Map<String, DeviceMetaData> deviceMetaDataMap = deviceService.getDeviceMetaDataMap(device, DeviceMetaAppendix.TEMP_WARN_GROUP);
                        ...
                        if (deviceStatus.temperatureAbnormalStatus == 2) {
                            warningEventDelay = getExecutionDelayTime(deviceStatus.statusUpdatedTime
                                    , LogistimoUtils.getMetaDataValue(deviceMetaDataMap.get(DeviceMetaAppendix.WARN_HIGH_DUR)));
                        } else if (deviceStatus.temperatureAbnormalStatus == 1) {
                            warningEventDelay = getExecutionDelayTime(deviceStatus.statusUpdatedTime
                                    ,LogistimoUtils.getMetaDataValue(deviceMetaDataMap.get(DeviceMetaAppendix.WARN_LOW_DUR)));
                        }
                      ...
                      Map<String, DeviceMetaData> deviceMetaDataMap = deviceService.getDeviceMetaDataMap(device, DeviceMetaAppendix.TEMP_ALARM_GROUP);
                        Long alarmEventDelay = 0L;
                        if (deviceStatus.temperatureAbnormalStatus == 2) {
                            alarmEventDelay = getExecutionDelayTime(deviceExcursionTime
                                    ,LogistimoUtils.getMetaDataValue(deviceMetaDataMap.get(DeviceMetaAppendix.ALARM_HIGH_DUR)));
                        } else if (deviceStatus.temperatureAbnormalStatus == 1) {
                            alarmEventDelay = getExecutionDelayTime(deviceExcursionTime
                                    , LogistimoUtils.getMetaDataValue(deviceMetaDataMap.get(DeviceMetaAppendix.ALARM_LOW_DUR)));
                        }
                ...
                LOGGER.warn("{} while scheduling temperature event for task options {}", e.getMessage(), options.toString(), e);
            } finally {
                if (LockUtil.shouldReleaseLock(lockStatus) && !LockUtil.release(String.valueOf(LogistimoConstant.DEVICE_LOCK + LogistimoUtils.extractDeviceId((String) options.get(DEVICE_ID))))) {
                    LOGGER.error("Error while releasing lock for the device {} in temperature event job", LogistimoUtils.extractDeviceId((String) options.get(DEVICE_ID)));
                }
            }
        }
    }
  ...
  private Long getExecutionDelayTime(Integer stateUpdatedTime, Integer duration){
    ...
    private void createTemperatureEvent(Device monitoringAsset) throws ServiceException {
        ...
        try{
            assetMapping = AssetMapping.findAssetMappingByRelatedAssetAndType(monitoringAsset, LogistimoConstant.MONITORED_BY);
        }catch (NoResultException e){
            return;
        }

        DeviceStatus deviceStatus = deviceService.getOrCreateDeviceStatus(assetMapping.asset, assetMapping.monitoringPositionId, null, AssetStatusConstants.TEMP_STATUS_KEY, null);
        ...
        Map<String, String> assetMetaMap = deviceService.getDeviceMetaDataValueAsString(monitoringAsset, DeviceMetaAppendix.TMP_GROUP);
        if(assetMetaMap != null){
            if(assetMetaMap.containsKey(DeviceMetaAppendix.TMP_MIN)){
                deviceEvent.attrs.put(DeviceEventPushModel.TMP_MIN, assetMetaMap.get(DeviceMetaAppendix.TMP_MIN));
            }

            if(assetMetaMap.containsKey(DeviceMetaAppendix.TMP_MAX)){
                deviceEvent.attrs.put(DeviceEventPushModel.TMP_MAX, assetMetaMap.get(DeviceMetaAppendix.TMP_MAX));
            }
        }
        ...
        try {
            ...
            LOGGER.error("{} while scheduling task for posting temperature event to Logistics service, {}", e.getMessage(), deviceEvent.toString(), e);
        }
    }

    private void updateMonitoredAssetStatus(Device monitoringAsset, TemperatureEventType temperatureEventType){
        AssetMapping assetMapping;
        try{
            assetMapping = AssetMapping.findAssetMappingByRelatedAssetAndType(monitoringAsset, LogistimoConstant.MONITORED_BY);
        }catch (NoResultException e){
        ...
        DeviceStatus deviceStatus = deviceService.getOrCreateDeviceStatus(assetMapping.asset, assetMapping.monitoringPositionId, null, AssetStatusConstants.TEMP_STATUS_KEY, null);
        DeviceStatus relatedAssetStatus = deviceService.getOrCreateDeviceStatus(assetMapping.relatedAsset, null, null, AssetStatusConstants.TEMP_STATUS_KEY, null);
        try{
            AlarmLog oldAlarmLog = AlarmLog.getOpenAlarmLogs(assetMapping.asset, deviceStatus.statusUpdatedTime, assetMapping.monitoringPositionId);
