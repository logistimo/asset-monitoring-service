...
public class DeviceWorkingStatusMigrator extends BaseController {
    ...
    public static Result migrate() {
        int si = 0;
        while (true) {
            List<DeviceStatus> ds = JPA.em().createQuery("from DeviceStatus where statusKey = 'wsk'", DeviceStatus.class)
                    .setFirstResult(si)
            ...
            for (DeviceStatus d : ds) {
                ...
                try {
                    ...
                    LOGGER.error("{} while scheduling task for posting working status update to Logistics service, {}", e.getMessage(), deviceEvent.toString(), e);
                }
            }
