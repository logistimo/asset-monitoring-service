...
public abstract class ZooElectableClient implements Watcher, ZkDataMonitor.ZkNodeDataListener {
    ...
    private final static Logger.ALogger LOGGER = Logger.of(ZooElectableClient.class);
    ...
    protected ZooElectableClient() throws KeeperException, IOException, InterruptedException {
        this.zooHosts = Play.application().configuration().getString("zoo.servers", "localhost:2181");
        this.timeout = Integer.parseInt(Play.application().configuration().getString("zoo.timeout", "3000"));
        this.electionZNodePath = Play.application().configuration().getString("zoo.path", "/election");
       init();
    }
    ...
    protected ZooElectableClient(String zooHosts, String electionZNodePath, int timeout) throws InterruptedException, IOException, KeeperException {
    ...
    protected ZooElectableClient(String electionZNodePath) throws InterruptedException, IOException, KeeperException {
        this.electionZNodePath = electionZNodePath;
        this.zooHosts = Play.application().configuration().getString("zoo.servers", "localhost:2181");
        this.timeout = Integer.parseInt(Play.application().configuration().getString("zoo.timeout", "3000"));
        init();
    ...
    private final String getElectionZNodePath() { return electionZNodePath; }
    ...
    public void onZNodeDeleted() {
        try {
            ...
            LOGGER.error("Failed to perform role ", e);
        }
    }
    ...
    public void onZooKeeperReconnected() {
        try {
            LOGGER.info("Zookeeper reconnected: performing role");

            //Determining role on reconnect
            determineAndPerformRole();
        } catch (Exception e) {
            LOGGER.error("{0} while performing role on zookeeper reconnect.", e.getMessage(), e);
        }
    }
