...
public class DevicePowerTransition {
    ...
    public static List<DevicePowerTransition> getDevicePowerTransition(Device device, Integer from, Integer to) {
        ...
        devicePowerTransitionList.addAll(JPA.em()
                .createQuery("from DevicePowerTransition where device = ?1 and time > ?2 and time <= ?3 order by time desc", DevicePowerTransition.class)
        ...
        DevicePowerTransition devicePowerTransition = JPA.em().createQuery("from DevicePowerTransition where device = ?1 and time <= ?2 order by time desc", DevicePowerTransition.class)
                .setParameter(1, device)
                .setParameter(2, from)
                .setMaxResults(1)
                .getSingleResult();
    ...
    public static DevicePowerTransition getRecentDevicePowerTransition(Device device, Integer to){
        return JPA.em().createQuery("from DevicePowerTransition where device = ?1 and time <= ?2 order by time desc", DevicePowerTransition.class)
                .setParameter(1, device)
                .setParameter(2, to)
                .setMaxResults(1)
                .getSingleResult();
