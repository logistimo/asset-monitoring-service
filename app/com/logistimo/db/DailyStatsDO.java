...
public class DailyStatsDO {
  ...
  public static List<DailyStatsDO> getDailyStats(Device device, int offset, int limit, int from,
                                                 int to) {
    if (from > 0 && to > 0) {
      return JPA.em()
          .createQuery(
              "from DailyStatsDO where device = ?1 and day >= ?2 and day < ?3 order by day desc",
              DailyStatsDO.class)
  ...
  public static int getDailyStatsCount(Device device, int from, int to) {
    if (from > 0 && to > 0) {
      return new BigInteger(JPA.em()
          .createNativeQuery(
              "select  count(1) from daily_stats where device_id = ?1 and day > ?2 and day < ?3")
          .setParameter(1, device)
          .setParameter(2, from)
          .setParameter(3, to)
