...
public class Device {
    ...
    @JoinTable(name = "devices_tags", joinColumns = {@JoinColumn(name = "devices_id")}, inverseJoinColumns = {@JoinColumn(name = "tags_id")})
    ...
    public static int getAbnormalReadingCountForDevices(List<Device> deviceList) {
        return new BigInteger(JPA.em()
                .createNativeQuery("select count(1) from devices where id in :inclList and temperatureState = ?1")
    ...
    public static List<Device> getActiveSensorDevices(){
        return JPA.em().createNativeQuery("select * from devices where assetType_id = 4 and id in" +
                "(select device_id from device_status where status_key = 'dsk_4' and status = 0) order by vendorId", Device.class)
                .getResultList();
    }

    public static int getInactiveCountForDevices(List<Device> deviceList){
        return new BigInteger(JPA.em()
                .createNativeQuery("select count(1) from devices where id in :inclList and activityState = ?1")
                .setParameter("inclList", deviceList)
                .setParameter(1, Device.DEVICE_ABNORMAL)
                .getSingleResult().toString()).intValue();
    }
